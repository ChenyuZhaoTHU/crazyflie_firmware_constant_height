...
def process_imu_data(timestamp, data, logconf):
    global imu_data
    q_x = data['stateEstimate.qx']
    q_y = data['stateEstimate.qy']
    q_z = data['stateEstimate.qz']
    q_w = data['stateEstimate.qw']
    ...
    imu_data = np.vstack((imu_data, [q_x, q_y, q_z, q_w, quat, current_time]))
...
if __name__ == '__main__':
    ...
    with SyncCrazyflie(URI) as scf:
        ...
        # Add IMU log config version 1
        imu_log_config = LogConfig(name='imu_data', period_in_ms=10)
        imu_log_config.add_variable('stateEstimate.qx', 'float')
        imu_log_config.add_variable('stateEstimate.qy', 'float')
        imu_log_config.add_variable('stateEstimate.qz', 'float')
        imu_log_config.add_variable('stateEstimate.qw', 'float')
