...
def load_data():
    ...
    train_data = train_data.reshape((90,-1))
...
class Train():
    ...
    def train(self):
    # train model
        history_usr1 = self.model_usr1.model.fit(self.input1_train[:,:,:,:], self.label1[:,:], epochs=900, batch_size = 2, 
                            validation_split=0.35, verbose=1, shuffle=True)
...
def test_n():
    ...
    ooout1 = model_pre1.predict(load_data()[10:12,:,:,:])
    ooout2 = model_pre1.predict(load_data()[53:55,:,:,:])
    ooout3 = model_pre1.predict(load_data()[86:89,:,:,:])
    # print(np.shape(test_label1))
...
if __name__ == "__main__":
    ...
    input1_train = load_data()
    kmeans = KMeans(n_clusters=3)
    kmeans.fit(input1_train)
